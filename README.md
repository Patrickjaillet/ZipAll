...
- **Pick a source folder, name the archive, pick a destination** — three native folder/file dialogs, no typing full paths by hand.
...
4. **Exclusions** (optional) — click *Add File(s)* or *Add Folder* to exclude specific files or subfolders from the archive; select an entry and click *Remove* to drop it.
5. **Compression** — choose *Stored* (fastest, no compression) or *Deflate* (smaller archives; the default).
...
```
src/ZipAll.Core/   Core library: directory walker, exclusion engine, ZIP writer/verifier (no UI dependency)
